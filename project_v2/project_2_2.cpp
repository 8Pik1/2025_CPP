...
{
    edge_to.assign((size_t)n + 1, vector<uint32_t>());
    edge_mode.assign((size_t)n + 1, vector<uint32_t>());
    edge_base_time.assign((size_t)n + 1, vector<uint32_t>());
...
void add_edge(
    ...
    long double load)
{
    edge_to[from].push_back((uint32_t)to);
    edge_mode[from].push_back((uint32_t)mode);
    edge_base_time[from].push_back((uint32_t)base_time);
    edge_load[from].push_back(load);

    edge_to[to].push_back((uint32_t)from);
    edge_mode[to].push_back((uint32_t)mode);
    edge_base_time[to].push_back((uint32_t)base_time);
    edge_load[to].push_back(load);
}

long double calculate_actual_time(
    uint32_t base_time,
    long double load,
    uint32_t mode)
{
    return (long double)base_time * (1.0L + load * sensitivity[(int)mode]);
}
    ...
    bool use_filter)
{
    ...
    while (!st.empty())
    {
        ...
        for (int i = 0; i < deg; i++)
        {
            if (use_filter && (int)edge_mode[v][i] != mode_filter)
            {
                continue;
            }

            int to = (int)edge_to[v][i];
    ...
    vector<vector<int>>& transfers)
{
    ...
    while (!heap_time.empty())
    {
        ...
        if (!heap_pop(cur_t, u, last_mode, heap_time, heap_station, heap_mode))
        {
            break;
        }

        if (cur_t != dist[u][last_mode])
        {
            continue;
        }
        ...
        for (int i = 0; i < deg; i++)
        {
            int v = (int)edge_to[u][i];
            uint32_t edge_m_u32 = edge_mode[u][i];
            uint32_t base_t = edge_base_time[u][i];
            long double load_val = edge_load[u][i];

            int edge_m = (int)edge_m_u32;

            long double actual = calculate_actual_time(base_t, load_val, edge_m_u32);
    ...
    const vector<long double>& comfort)
{
    ...
    if (time_total[a] != time_total[b])
    {
        return time_total[a] < time_total[b];
    }

    if (transfers_cnt[a] != transfers_cnt[b])
    {
        return transfers_cnt[a] < transfers_cnt[b];
    }
    ...
    const vector<int>& modes)
{
    ...
    {
        int used_mode = modes[i];
        cout << " -" << used_mode << "-> " << stations[i];
    }
}

// ============================================================================
// 6. Ввод (io_module.cpp)
// ============================================================================
...
bool read_input()
{
    ...
    graph_init((uint32_t)N);

    for (int i = 0; i < MODES; i++)
    {
        if (!(cin >> sensitivity[i]))
        {
            return false;
        }
    }
    ...
    for (int e = 0; e < M; e++)
    {
        ...
        if (!(cin >> from >> to >> mode >> base_time >> load))
        {
            return false;
        }

        if (!station_ok(from) || !station_ok(to))
        {
            return false;
        }
...
int main()
{
    ...
    for (int query_idx = 1; query_idx <= Q; query_idx++)
    {
        ...
        if (!(cin >> start_station >> K >> coef))
        {
            return 0;
        }

        if (!station_ok(start_station))
        {
            return 0;
        }
        ...
        if (!result_destinations.empty())
        {
            quick_sort_results(
                0,
                (int)result_destinations.size() - 1,
                result_destinations,
                result_reachable,
                result_times,
                result_transfers,
